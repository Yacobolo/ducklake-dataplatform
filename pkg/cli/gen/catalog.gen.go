...
func newCatalogCmd(client *Client) *cobra.Command {
	...
	// createTable
	{
		...
		c.Flags().String("comment", "", "Comment")
		c.Flags().String("file-format", "parquet", "File format of the external data. Required for EXTERNAL tables. (one of: parquet, csv)")
		c.Flags().String("json", "", "JSON input (raw string or @filename or - for stdin)")
		...
		c.Flags().String("table-type", "MANAGED", "Type of table. MANAGED (default) or EXTERNAL. (one of: MANAGED, EXTERNAL)")
	...
	// createView
	{
		c := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				if jsonInput == "" {
					...
					if !cmd.Flags().Changed("view-definition") {
						return fmt.Errorf("required flag %q not set (or use --json)", "view-definition")
					}
				}
		...
		c.Flags().String("json", "", "JSON input (raw string or @filename or - for stdin)")
		c.Flags().String("name", "", "Name")
		c.Flags().String("view-definition", "", "View definition")
	...
	// createVolume
	{
		c := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				if jsonInput == "" {
					...
					if !cmd.Flags().Changed("volume-type") {
						return fmt.Errorf("required flag %q not set (or use --json)", "volume-type")
					}
				}
		...
		c.Flags().String("volume-type", "", "Volume type (one of: MANAGED, EXTERNAL)")
		...
		c := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				default:
					...
					columns := []string{"id", "name", "metastore_type", "status", "comment", "is_default", "status_message"}
					rows := ExtractRows(data, columns)
					PrintTable(os.Stdout, columns, rows)
				}
				return nil
	...
	// registerCatalog
	{
		c := &cobra.Command{
			...
			Example: "duck catalog register analytics --comment \"Analytics data catalog\" --data-path s3://my-bucket/analytics/ --dsn /data/analytics.db --metastore-type sqlite",
			Args:    cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				outputFlag, _ := cmd.Flags().GetString("output")
				_ = outputFlag
				...
				if jsonInput == "" {
					if !cmd.Flags().Changed("data-path") {
						return fmt.Errorf("required flag %q not set (or use --json)", "data-path")
					}
					if !cmd.Flags().Changed("dsn") {
						return fmt.Errorf("required flag %q not set (or use --json)", "dsn")
					}
					if !cmd.Flags().Changed("metastore-type") {
						return fmt.Errorf("required flag %q not set (or use --json)", "metastore-type")
					}
				}
		...
		c.Flags().String("comment", "", "Comment")
		c.Flags().String("data-path", "", "Data path")
		c.Flags().String("dsn", "", "Dsn")
		c.Flags().String("json", "", "JSON input (raw string or @filename or - for stdin)")
		c.Flags().String("metastore-type", "", "Metastore type (one of: sqlite, postgres)")
	...
	// setDefaultCatalog
	{
		c := &cobra.Command{
			...
			Example: "duck catalog set-default <catalog-name>",
			Args:    cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				outputFlag, _ := cmd.Flags().GetString("output")
				_ = outputFlag
	...
	{
		c := &cobra.Command{
			Use:     "update-registration <catalog-name>",
			Short:   "Update a catalog registration",
			Long:    "Updates the registration details for a catalog.",
			Example: "duck catalog update-registration <catalog-name> --comment \"Updated analytics catalog\" --data-path s3://new-bucket/analytics/",
			Args:    cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				outputFlag, _ := cmd.Flags().GetString("output")
				_ = outputFlag
	...
	// updateVolume
	{
		...
		c.Flags().String("comment", "", "Comment")
		c.Flags().String("json", "", "JSON input (raw string or @filename or - for stdin)")
		c.Flags().String("new-name", "", "New name")
		c.Flags().String("owner", "", "Owner")
