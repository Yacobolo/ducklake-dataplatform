...
func allAPIEndpoints() []gen.APIEndpoint {
	if len(gen.APIGeneratedEndpoints) == 0 {
		return gen.APIEndpoints
	}

	combined := make([]gen.APIEndpoint, 0, len(gen.APIEndpoints)+len(gen.APIGeneratedEndpoints))
	combined = append(combined, gen.APIEndpoints...)
	seen := make(map[string]struct{}, len(combined))
	for _, endpoint := range combined {
		seen[endpoint.OperationID] = struct{}{}
	}
	for _, endpoint := range gen.APIGeneratedEndpoints {
		if _, ok := seen[endpoint.OperationID]; ok {
			continue
		}
		combined = append(combined, gen.APIEndpoint{
			OperationID: endpoint.OperationID,
			Method:      endpoint.Method,
			Path:        endpoint.Path,
			Tags:        endpoint.Tags,
			Summary:     "Generated by apigen",
		})
	}
	return combined
}
