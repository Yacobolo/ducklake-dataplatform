...
Use this quickstart when you are responsible for platform posture. The goal is not just to make QuackStack start, but to make it safe, governable, and observable.
...
<figure class="my-8 overflow-x-auto rounded-[1.5rem] border border-[var(--borderColor-default)] bg-[var(--bgColor-inset)] p-5">
  <img class="mx-auto block h-auto w-max max-w-none rounded-none border-0 bg-transparent" src="/_site/diagrams/control-plane-remote-compute.svg" alt="Topology diagram showing principals reaching the QuackStack control plane, policy enforcement, local execution, optional remote workers, and storage." loading="lazy" decoding="async">
</figure>
