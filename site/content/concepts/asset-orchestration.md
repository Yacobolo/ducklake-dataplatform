...
If you only remember one thing, remember this: an asset in QuackStack is a durable output that the platform knows how to track, refresh, validate, and explain operationally.
...
QuackStack uses assets to describe outputs that matter operationally. The asset layer sits above raw SQL and above a single run of a query. It tells the platform how an output should behave over time, not just how it was produced once.

## What An Asset Is

In QuackStack, an asset is a named output in the dependency graph that the platform can monitor and act on. An asset might represent a table, a view, a model output, a notebook output, a semantic object, or another durable result that downstream users depend on.
...
An asset definition is the metadata that tells QuackStack how to treat that output. In plain English, it answers:
...
Those fields matter because they change what the platform can explain and automate. Without an asset definition, QuackStack may know an output exists. With an asset definition, QuackStack can tell you why it is stale, which upstream failed, what partitions are missing, and which remediation action makes sense.
...
A run is an execution record. It answers, “what happened when QuackStack tried to do the work?” A run has a status such as pending, running, success, or failed. Runs are about execution history.
...
A reconcile action is QuackStack’s way of saying, “given the current freshness state and dependency graph, what should be rematerialized or repaired to move this output back toward healthy?” Reconcile does not mean “blindly rerun everything.” It means analyzing the graph and identifying the nearest useful recovery targets.

## Partitions And Dependency Graphs

Partitions matter when an asset is produced in slices rather than all at once, such as by day, region, or customer segment. Partitioned assets let QuackStack reason about partial freshness and partial repair. Instead of saying “the whole asset is broken,” QuackStack can say “the 2026-03-12 partition is missing” or “only the APAC slice needs a backfill.”

The dependency graph is the map that connects assets to the assets they rely on. This graph is what lets QuackStack explain blockers, freshness propagation, and downstream impact. Without the graph, every failure looks isolated. With the graph, the platform can tell a coherent operational story.
...
One morning the dashboard looks stale. QuackStack can now answer a chain of questions:
...
- [Pipelines API](/reference/generated/api/endpoints/pipelines)
