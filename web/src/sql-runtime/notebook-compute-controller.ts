...
function renderNotebookResult(cell: HTMLElement, columns: string[], rows: unknown[][], rowCount: number): void {
  ...
  target.innerHTML = [
    ...
    '<h4 class="m-0 text-muted">Output</h4>',
    "</div>",
    `<p class="m-0 text-sm text-muted">${rowCount} row(s) · Local DuckDB WASM</p>`,
    `<div class="overflow-x-auto"><table class="min-w-full border-collapse overflow-hidden rounded-xl border border-border bg-background [&_tbody_tr:hover]:bg-surface-muted [&_td]:border-b [&_td]:border-border [&_td]:px-4 [&_td]:py-3 [&_td]:align-top [&_td]:text-[0.8125rem] [&_th]:sticky [&_th]:top-0 [&_th]:z-[1] [&_th]:border-b [&_th]:border-border [&_th]:bg-surface-muted [&_th]:px-4 [&_th]:py-3 [&_th]:text-left [&_th]:text-[0.8125rem] [&_th]:font-semibold [&_th]:uppercase [&_th]:tracking-[0.02em] [&_th]:text-muted"><thead><tr>${header}</tr></thead><tbody>${body}</tbody></table></div>`,
  ].join("");
}
...
function renderNotebookLocalError(cell: HTMLElement, message: string): void {
  ...
  target.innerHTML = [
    '<div class="rounded-xl border border-border-danger bg-danger-muted p-4">',
    '<h4 class="m-0 text-muted">Local Query Unavailable</h4>',
...
function renderNotebookMessage(cell: HTMLElement, message: string): void {
  ...
  target.innerHTML = `<p class="m-0 text-sm text-muted">${escapeHTML(message)}</p>`;
}
