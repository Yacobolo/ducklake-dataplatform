...
func TestAgentHandler_HealthAndMetrics(t *testing.T) {
	...
	t.Run("health", func(t *testing.T) {
		...
		assert.InDelta(t, float64(1), health["active_queries"], 0.0)
		assert.InDelta(t, float64(2), health["queued_jobs"], 0.0)
		assert.InDelta(t, float64(3), health["running_jobs"], 0.0)
		assert.InDelta(t, float64(4), health["completed_jobs"], 0.0)
		assert.InDelta(t, float64(5), health["stored_jobs"], 0.0)
		assert.InDelta(t, float64(6), health["cleaned_jobs"], 0.0)
		assert.InDelta(t, float64(8), health["max_memory_gb"], 0.0)
	})
