	...
	renderHTML(w, http.StatusOK, appPage("Storage Credential: "+item.Name, "storage", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			...
			Div(Class("BtnGroup"),
				...
				Form(Method("post"), Action("/ui/storage/credentials/"+url.PathEscape(item.Name)+"/delete"), csrfFieldProvider(r)(), Button(Type("submit"), Class("btn btn-danger"), Text("Delete"))),
			),
		),
	))
}
	...
	renderHTML(w, http.StatusOK, appPage("External Location: "+item.Name, "storage", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			...
			Div(Class("BtnGroup"),
				...
				Form(Method("post"), Action("/ui/storage/locations/"+url.PathEscape(item.Name)+"/delete"), csrfFieldProvider(r)(), Button(Type("submit"), Class("btn btn-danger"), Text("Delete"))),
			),
		),
	))
}
...
func (h *Handler) StorageVolumesList(w http.ResponseWriter, r *http.Request) {
	...
	filterCard := Div(Class(cardClass()),
		Form(Class("stack-form"), Method("get"), Action("/ui/storage/volumes"),
			Label(Text("Catalog")),
			Input(Name("catalog"), Value(catalogName)),
			Label(Text("Schema")),
			Input(Name("schema"), Value(schemaName)),
			Div(Class("form-actions"), Button(Type("submit"), Class(secondaryButtonClass()), Text("Load volumes"))),
		),
	)
	...
	renderHTML(w, http.StatusOK, appPage("Volume: "+item.Name, "storage", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			...
			Div(Class("BtnGroup"),
				...
				Form(Method("post"), Action("/ui/storage/volumes/"+url.PathEscape(catalogName)+"/"+url.PathEscape(schemaName)+"/"+url.PathEscape(volumeName)+"/delete"), csrfFieldProvider(r)(), Button(Type("submit"), Class("btn btn-danger"), Text("Delete"))),
			),
		),
	))
}
...
func (h *Handler) ComputeEndpointsDetail(w http.ResponseWriter, r *http.Request) {
	...
	for i := range assignments {
		...
		assignmentRows = append(assignmentRows, Tr(
			...
			Td(Class("text-right"), Form(Method("post"), Action("/ui/compute/endpoints/"+url.PathEscape(name)+"/assignments/"+url.PathEscape(itemAssignment.ID)+"/delete"), csrfFieldProvider(r)(), Button(Type("submit"), Class("btn btn-danger"), Text("Remove")))),
		))
	}
	renderHTML(w, http.StatusOK, appPage("Compute Endpoint: "+item.Name, "compute", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			...
			Div(Class("BtnGroup"),
				...
				Form(Method("post"), Action("/ui/compute/endpoints/"+url.PathEscape(name)+"/delete"), csrfFieldProvider(r)(), Button(Type("submit"), Class("btn btn-danger"), Text("Delete"))),
			),
		),
		Div(Class(cardClass()),
			H2(Text("Create assignment")),
			Form(Class("stack-form"), Method("post"), Action("/ui/compute/endpoints/"+url.PathEscape(name)+"/assignments"),
				csrfFieldProvider(r)(),
				...
				Div(Class("form-actions"), Button(Type("submit"), Class(primaryButtonClass()), Text("Create assignment"))),
			),
		),
		Div(Class(cardClass("table-wrap")),
			H2(Text("Assignments")),
			Table(Class("data-table"), THead(Tr(Th(Text("Principal ID")), Th(Text("Type")), Th(Text("Default")), Th(Text("Fallback Local")), Th(Class("text-right"), Text("Actions")))), TBody(Group(assignmentRows))),
		),
	))
}
...
func (h *Handler) GovernanceTagsList(w http.ResponseWriter, r *http.Request) {
	...
	renderHTML(w, http.StatusOK, appPage("Governance: Tags", "governance", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			H2(Text("Create tag")),
			Form(Class("stack-form"), Method("post"), Action("/ui/governance/tags"),
				csrfFieldProvider(r)(),
				Label(Text("Key")),
				Input(Name("key"), Required()),
				Label(Text("Value")),
				Input(Name("value")),
				Div(Class("form-actions"), Button(Type("submit"), Class(primaryButtonClass()), Text("Create tag"))),
			),
		),
		Div(Class(cardClass()),
			H2(Text("Assign tag")),
			Form(Class("stack-form"), Method("post"), Action("/ui/governance/tag-assignments"),
				csrfFieldProvider(r)(),
				Label(Text("Tag ID")),
				...
				Div(Class("form-actions"), Button(Type("submit"), Class(secondaryButtonClass()), Text("Assign tag"))),
			),
		),
		Div(Class(cardClass()),
			H2(Text("Remove tag assignment")),
			Form(Class("stack-form"), Method("post"), Action("/ui/governance/tag-assignments/delete"),
				csrfFieldProvider(r)(),
				Label(Text("Assignment ID")),
				Input(Name("assignment_id"), Required()),
				Div(Class("form-actions"), Button(Type("submit"), Class("btn btn-danger"), Text("Remove assignment"))),
			),
		),
		Div(Class(cardClass("table-wrap")),
			Table(Class("data-table"), THead(Tr(Th(Text("Key")), Th(Text("Value")), Th(Text("Created By")), Th(Text("Assignments")), Th(Class("text-right"), Text("Actions")))), TBody(Group(rows))),
		),
...
func (h *Handler) GovernanceLineage(w http.ResponseWriter, r *http.Request) {
	...
	renderHTML(w, http.StatusOK, appPage("Governance: Lineage", "governance", principalFromContext(r.Context()),
		...
		Div(Class(cardClass()),
			Form(Class("stack-form"), Method("get"), Action("/ui/governance/lineage"),
				Label(Text("Schema")),
				...
				Div(Class("form-actions"), Button(Type("submit"), Class(primaryButtonClass()), Text("Load lineage"))),
			),
		),
		Div(Class(cardClass()),
			H2(Text("Purge lineage")),
			Form(Class("stack-form"), Method("post"), Action("/ui/governance/lineage/purge"),
				csrfFieldProvider(r)(),
				Label(Text("Delete edges older than days")),
				Input(Name("older_than_days"), Value("30")),
				Div(Class("form-actions"), Button(Type("submit"), Class("btn btn-danger"), Text("Purge lineage"))),
			),
		),
		Div(Class(cardClass("table-wrap")), H2(Text("Upstream")), Table(Class("data-table"), THead(Tr(Th(Text("Source")), Th(Text("Target")), Th(Text("Type")), Th(Class("text-right"), Text("Actions")))), TBody(Group(upstreamRows)))),
		Div(Class(cardClass("table-wrap")), H2(Text("Downstream")), Table(Class("data-table"), THead(Tr(Th(Text("Source")), Th(Text("Target")), Th(Text("Type")), Th(Class("text-right"), Text("Actions")))), TBody(Group(downstreamRows)))),
		Div(Class(cardClass("table-wrap")), H2(Text("Column lineage")), Table(Class("data-table"), THead(Tr(Th(Text("Target Column")), Th(Text("Source Column")), Th(Text("Transform")), Th(Text("Function")))), TBody(Group(columnRows)))),
		Div(Class(cardClass("table-wrap")), H2(Text("Column impact")), Table(Class("data-table"), THead(Tr(Th(Text("Source Column")), Th(Text("Target Column")), Th(Text("Transform")), Th(Text("Function")))), TBody(Group(impactRows)))),
	))
}
