...
func notebookDetailPage(d notebookDetailPageData) Node {
	...
	for i := range d.Cells {
		...
		if isMarkdown {
			cellFrameClass = "relative overflow-hidden rounded-xl border border-transparent bg-transparent before:absolute before:inset-y-0 before:left-0 before:w-[2px] before:bg-[var(--borderColor-accent-emphasis)] before:opacity-0 before:transition-opacity [.is-editing-markdown_&]:border-[var(--borderColor-muted)] [.is-editing-markdown_&]:before:opacity-100"
		...
		if c.CellType == string(domain.CellTypeSQL) {
			runButton = Button(
				...
				Attr("form", formID),
				FormAction(c.RunURL),
				Class("inline-flex min-h-8 min-w-8 items-center justify-center rounded-lg border border-transparent bg-transparent px-2 text-[var(--fgColor-success)] hover:border-[var(--borderColor-muted)] hover:bg-[var(--bgColor-muted)]"),
		...
		cellMenuItems := []Node{
			Form(Method("post"), Action(c.MoveURL), d.CSRFFieldFunc(), Input(Type("hidden"), Name("direction"), Value("up")), Button(Type("submit"), Class(dropdownItemClass("text-[var(--fgColor-default)]")), Text("Move cell up"))),
			Form(Method("post"), Action(c.MoveURL), d.CSRFFieldFunc(), Input(Type("hidden"), Name("direction"), Value("down")), Button(Type("submit"), Class(dropdownItemClass("text-[var(--fgColor-default)]")), Text("Move cell down"))),
			Form(Method("post"), Action("/ui/notebooks/"+d.NotebookID+"/cells"), d.CSRFFieldFunc(), Input(Type("hidden"), Name("cell_type"), Value("sql")), Input(Type("hidden"), Name("content"), Value("")), Input(Type("hidden"), Name("position"), Value(strconv.Itoa(c.Position))), Button(Type("submit"), Class(dropdownItemClass("text-[var(--fgColor-default)]")), Attr("data-add-above", "true"), Text("Insert SQL cell above"))),
			Form(Method("post"), Action("/ui/notebooks/"+d.NotebookID+"/cells"), d.CSRFFieldFunc(), Input(Type("hidden"), Name("cell_type"), Value("sql")), Input(Type("hidden"), Name("content"), Value("")), Input(Type("hidden"), Name("position"), Value(strconv.Itoa(c.Position+1))), Button(Type("submit"), Class(dropdownItemClass("text-[var(--fgColor-default)]")), Attr("data-add-below", "true"), Text("Insert SQL cell below"))),
		...
		cellNodes = append(cellNodes, Article(
			Class("group/notebook-cell relative scroll-mt-[4.5rem] px-1 transition-[background-color,box-shadow] [&.dragging]:opacity-75 [&.drag-over]:shadow-[inset_0_0_0_2px_var(--borderColor-accent-emphasis)]"),
			ID("cell-"+c.ID),
			...
			Div(Class("grid gap-2 [grid-template-columns:2rem_minmax(0,1fr)_3rem] max-md:grid-cols-[auto_1fr_auto]"),
				Aside(Class("flex flex-col items-center gap-2 pt-2 max-md:col-start-1 max-md:row-start-1 max-md:flex-row max-md:justify-start max-md:pt-0"), runButton, Span(Class("font-mono text-xs text-[var(--fgColor-muted)]"), Text(strconv.Itoa(c.Position+1)))),
		...
		outlineIndentClass := ""
		if outlineLevel == 2 {
			outlineIndentClass = "pl-3"
		} else if outlineLevel >= 3 {
			outlineIndentClass = "pl-4 [&_.notebook-outline-label]:text-xs"
		}
	...
	outlinePanel := Details(
		...
		Attr("open", "open"),
		Summary(Class("list-none [&::-webkit-details-marker]:hidden flex items-center justify-between gap-2 border-b border-[var(--borderColor-muted)] pb-2 text-sm font-semibold"),
			Span(Text("Outline")),
...
func notebookComputeCard(d notebookDetailPageData) Node {
	...
	return Div(
		Class("sql-compute-card rounded-xl border border-[var(--borderColor-default)] bg-[var(--bgColor-default)] p-4 shadow-xs"),
...
func notebookResultNode(c notebookCellRow) Node {
	...
	if c.VisualSpec != nil && c.VisualSpec.Kind != domain.VisualOutputTable {
		...
		case domain.VisualOutputMetric:
			...
			return Div(Class("notebook-output flex flex-col gap-3 rounded-xl border border-[var(--borderColor-default)] bg-[var(--bgColor-muted)] p-4"), Attr("data-notebook-cell-output", "true"), visualMetricCard(defaultVisualTitle(c.VisualSpec, "Metric"), value, secondary), notebookResultDataDetails(c))
		case domain.VisualOutputChart:
			return Div(Class("notebook-output flex flex-col gap-3 rounded-xl border border-[var(--borderColor-default)] bg-[var(--bgColor-muted)] p-4"), Attr("data-notebook-cell-output", "true"), chartHost(c.LastResult.Columns, c.LastResult.RawRows, c.VisualSpec), notebookResultDataDetails(c))
		}
	}
