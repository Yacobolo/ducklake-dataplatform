...
func notebookDetailPage(d notebookDetailPageData) Node {
	...
	for i := range d.Cells {
		...
		outlineNodes = append(outlineNodes, Li(
			...
			A(Href("#cell-"+c.ID), Class(core.ClassNames("flex items-center justify-between gap-2 rounded-lg px-2 py-2 text-sm text-[var(--fgColor-default)] no-underline transition-colors visited:text-[var(--fgColor-default)] hover:text-[var(--fgColor-accent)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[var(--outline-focus-offset)] focus-visible:outline-[var(--focus-outlineColor)] [&.is-active-outline-link]:text-[var(--fgColor-accent)]", outlineIndentClass)), Attr("data-outline-link", "true"), Attr("data-cell-anchor", "cell-"+c.ID), Attr("data-outline-level", strconv.Itoa(outlineLevel)),
