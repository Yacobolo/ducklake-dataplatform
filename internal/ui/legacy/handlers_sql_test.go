...
func TestUINotebookRuntimeManifest_RequiresWebSession(t *testing.T) {
	router, _ := newUITestRouter(t)

	req := httptest.NewRequest(http.MethodGet, "/ui/notebooks/runtime/manifest?schema=main&table=items", nil)
...
func TestUINotebookRuntimeManifest_ReturnsJSONErrorWhenServiceMissing(t *testing.T) {
	router, _ := newUITestRouter(t)
...
func TestUINotebookRuntimeManifest_RequiresTableParameter(t *testing.T) {
	router := newUISQLManifestRouter(t, &query.ManifestService{})
	sessionCookie := loginSessionCookie(t, router)

	req := httptest.NewRequest(http.MethodGet, "/ui/notebooks/runtime/manifest?schema=main", nil)
	req.AddCookie(sessionCookie)
	resp := httptest.NewRecorder()
	router.ServeHTTP(resp, req)
