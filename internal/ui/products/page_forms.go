...
func productNewPage(principal domain.ContextPrincipal, csrfField func() Node) Node {
	return core.AppPage(
		...
		Div(Class("flex flex-col gap-4"),
			Div(Class("flex flex-col gap-4 rounded-xl border border-border bg-background p-4 shadow-xs"),
				Div(Class("flex flex-wrap items-start justify-between gap-3"),
					Div(Class("flex min-w-0 flex-col gap-1"),
						H2(Class("m-0 text-lg font-semibold"), Text("Create product")),
						P(Class("text-sm text-muted"), Text("Create a draft product, normalize ownership into domain/team records, and optionally link a primary runtime asset.")),
					),
				),
