...
func TestRenderCatalogWorkspace_ViewColumns(t *testing.T) {
	t.Run("renders discovered view columns on overview", func(t *testing.T) {
		...
		assert.Contains(t, body, `data-column-type-group="integer"`)
		assert.Contains(t, body, `data-column-type-group="floating"`)
		assert.Contains(t, body, `data-column-nullable="false"`)
		assert.Contains(t, body, `data-column-nullable="true"`)
		assert.Contains(t, body, "REQUIRED")
		assert.Contains(t, body, "NULLABLE")
