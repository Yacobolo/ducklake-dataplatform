...
func modelsListPage(principal domain.ContextPrincipal, rows []modelsListRowData, page domain.PageRequest, total int64) Node {
	table := Node(P(Class(core.MutedClass()), Text("No models available.")))
	if len(rows) > 0 {
...
func modelsDetailPage(d modelsDetailPageData) Node {
	tests := Node(P(Class(core.MutedClass()), Text("No tests configured.")))
	if len(d.Tests) > 0 {
		...
		for i := range d.Tests {
			...
			rows = append(rows, Tr(
				...
				Td(Class("text-right"), Form(Method("post"), Action(t.DeleteURL), d.CSRFFieldProvider(), Button(Type("submit"), Class(core.DangerButtonClass("small")), Text("Delete")))),
			))
		}
		tests = Div(Class(core.TableWrapClass()), Table(Class("min-w-full text-left text-sm"),
			THead(Tr(Th(Text("Name")), Th(Text("Type")), Th(Text("Column")), Th(Class("text-right"), Text("Actions")))),
			TBody(Group(rows)),
		))
	}

	freshness := Node(P(Class(core.MutedClass()), Text("No freshness policy configured.")))
	...
	return core.AppPage("Model: "+d.QualifiedName, "models", d.Principal,
		Div(Class(core.CardClass()),
			...
			Div(Class(core.ButtonRowClass()),
				...
				A(Href(d.NewTestURL), Class(core.PrimaryButtonClass()), Text("New test")),
				A(Href(d.RunsURL), Class(core.SecondaryButtonClass()), Text("Runs")),
				A(Href(d.DAGURL), Class(core.SecondaryButtonClass()), Text("DAG")),
				A(Href(d.SourceFreshnessURL), Class(core.SecondaryButtonClass()), Text("Source freshness")),
		...
		Div(Class(core.CardClass()),
			...
			freshness,
			Form(Method("post"), Action(d.FreshnessURL), d.CSRFFieldProvider(), Button(Type("submit"), Class(core.SecondaryButtonClass()), Text("Refresh freshness status"))),
		),
		Div(Class(core.CardClass()),
			...
			Form(Class("grid gap-3"), Method("post"), Action(d.TriggerRunURL),
				...
				Input(Name("target_catalog"), Class(core.FormControlClass()), Required()),
				Label(Text("Target schema")),
				Input(Name("target_schema"), Class(core.FormControlClass()), Required()),
				Label(Text("Selector")),
				Input(Name("selector"), Class(core.FormControlClass()), Value(d.DefaultSelector)),
				Button(Type("submit"), Class(core.PrimaryButtonClass()), Text("Trigger model run")),
			),
		),
		Div(Class(core.CardClass()),
			...
			Form(Class("grid gap-3"), Method("post"), Action(d.CancelRunURL),
				...
				Input(Name("run_id"), Class(core.FormControlClass())),
				Button(Type("submit"), Class(core.SecondaryButtonClass()), Text("Cancel model run")),
			),
		),
		Div(Class(core.CardClass()),
...
func modelsNewPage(principal domain.ContextPrincipal, csrfFieldProvider func() Node) Node {
	return modelFormPage(principal, "New Model", "/ui/models", csrfFieldProvider,
		...
		Select(Name("materialization"), Class(core.FormControlClass()),
		...
		Label(Text("SQL")),
		Textarea(Name("sql"), Required(), Class(core.FormControlClass("min-h-40 font-mono text-xs"))),
	)
}
...
func modelsEditPage(principal domain.ContextPrincipal, projectName, modelName string, model *domain.Model, csrfFieldProvider func() Node) Node {
	return modelFormPage(principal, "Edit Model", "/ui/models/"+projectName+"/"+modelName+"/update", csrfFieldProvider,
		...
		Label(Text("Description")),
		Textarea(Name("description"), Class(core.FormControlClass("min-h-24")), Text(model.Description)),
		Label(Text("Tags (comma separated)")),
		Input(Name("tags"), Value(csvValues(model.Tags)), Class(core.FormControlClass())),
		Label(Text("SQL")),
		Textarea(Name("sql"), Required(), Class(core.FormControlClass("min-h-40 font-mono text-xs")), Text(model.SQL)),
	)
}
...
func modelTestsNewPage(principal domain.ContextPrincipal, projectName, modelName string, csrfFieldProvider func() Node) Node {
	return modelFormPage(principal, "New Model Test", "/ui/models/"+projectName+"/"+modelName+"/tests", csrfFieldProvider,
		...
		Input(Name("name"), Required(), Class(core.FormControlClass())),
		Label(Text("Type")),
		Select(Name("test_type"), Class(core.FormControlClass()),
		...
		Label(Text("Column")),
		Input(Name("column"), Class(core.FormControlClass())),
		Label(Text("Values (accepted_values, comma separated)")),
		Input(Name("values"), Class(core.FormControlClass())),
		Label(Text("To Model (relationships)")),
		Input(Name("to_model"), Class(core.FormControlClass())),
		Label(Text("To Column (relationships)")),
		Input(Name("to_column"), Class(core.FormControlClass())),
		Label(Text("SQL (custom_sql)")),
		Textarea(Name("test_sql"), Class(core.FormControlClass("min-h-32 font-mono text-xs"))),
	)
}
...
func modelsDAGPage(d modelsDAGPageData) Node {
	...
	if len(tierNodes) == 0 {
		tierNodes = append(tierNodes, Div(Class(core.CardClass()), P(Class(core.MutedClass()), Text("No model DAG available.")), A(Href("/ui/models"), Class(core.SecondaryButtonClass()), Text("Back to models"))))
	}
	title := "Model DAG"
	...
	content := append([]Node{
		Div(Class(core.CardClass()), A(Href("/ui/models"), Class(core.SecondaryButtonClass()), Text("Back to models"))),
	}, tierNodes...)
	return core.AppPage(title, "models", d.Principal, content...)
}

func modelRunsListPage(d modelRunsListPageData) Node {
	table := Node(P(Class(core.MutedClass()), Text("No model runs found.")))
	if len(d.Rows) > 0 {
	...
	return core.AppPage("Model Runs", "models", d.Principal,
		Div(Class(core.CardClass()), A(Href("/ui/models/dag"), Class(core.SecondaryButtonClass()), Text("View DAG"))),
		Div(Class(core.CardClass()),
			table,
...
func modelRunDetailPage(d modelRunDetailPageData) Node {
	...
	return core.AppPage("Model Run: "+d.RunID, "models", d.Principal,
		Div(Class(core.CardClass()),
			...
			Form(Method("post"), Action(d.CancelURL), d.CSRFFieldProvider(), Button(Type("submit"), Class(core.SecondaryButtonClass()), Text("Cancel run"))),
		),
		Div(Class(core.CardClass()), H2(Class("mt-0 text-lg font-semibold"), Text("Compile manifest")), Pre(Class("overflow-x-auto rounded-lg border border-[var(--borderColor-muted)] bg-[var(--bgColor-muted)] p-3 text-sm"), Text(d.CompileManifest))),
		Div(Class(core.CardClass()), H2(Class("mt-0 text-lg font-semibold"), Text("Steps")),
			Div(Class(core.TableWrapClass()), Table(Class("min-w-full text-left text-sm"),
...
func modelSourceFreshnessPage(d modelSourceFreshnessPageData) Node {
	resultNode := Node(nil)
	if d.Result != nil {
		resultNode = Div(Class(core.CardClass()),
	...
	return core.AppPage("Source Freshness", "models", d.Principal,
		Div(Class(core.CardClass()),
			...
			Form(Class("grid gap-3"), Method("post"), Action("/ui/models/source-freshness"),
				...
				Input(Name("source_schema"), Value(d.SourceSchema), Required(), Class(core.FormControlClass())),
				Label(Text("Source table")),
				Input(Name("source_table"), Value(d.SourceTable), Required(), Class(core.FormControlClass())),
				Label(Text("Timestamp column")),
				Input(Name("timestamp_column"), Value(d.TimestampColumn), Class(core.FormControlClass())),
				Label(Text("Max lag seconds")),
				Input(Name("max_lag_seconds"), Value(defaultString(d.MaxLagSecondsText, "3600")), Required(), Class(core.FormControlClass())),
				Button(Type("submit"), Class(core.PrimaryButtonClass()), Text("Check freshness")),
			),
		),
		resultNode,
	)
...
func modelFormPage(principal domain.ContextPrincipal, title, action string, csrfFieldProvider func() Node, fields ...Node) Node {
	...
	return core.AppPage(title, "models", principal, Div(Class(core.CardClass()), Form(Class("grid gap-3"), Method("post"), Action(action), Group(nodes))))
}
