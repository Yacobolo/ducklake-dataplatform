...
func TestSemanticHandlers_UseSharedDomainErrorResponder(t *testing.T) {
	...
	if !containsAny(source, []string{"respondDomainError[", "respondDomainErrorForOperation["}) {
		t.Fatal("governance: internal/api/handler_semantic.go must use shared domain error mapping")
	}
...
func TestComputeHandlers_UseSharedDomainErrorResponder(t *testing.T) {
	...
	source := string(body)
	if !containsAny(source, []string{"respondDomainError[", "respondDomainErrorForOperation["}) {
		t.Fatal("governance: internal/api/handler_compute.go must use shared domain error mapping helpers")
	}
	if containsAny(source, []string{"errors.As(err,"}) {
