...
// Ensure the mock satisfies the interface with the correct CheckPrivilege signature.
// The CheckPrivilege method signature in mockAuthService takes (ctx, principalName, securableType string, securableID int64, privilege string).
// We need to fix allowManageCompute/denyManageCompute since the actual func signature is different.
