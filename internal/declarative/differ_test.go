...
func TestDiff_DataProducts(t *testing.T) {
	desired := &DesiredState{
		Domains: []DomainResource{{Name: "revenue", Spec: DomainSpec{Description: "Revenue"}}},
		Teams:   []TeamResource{{Name: "analytics-engineering", Spec: TeamSpec{DomainRef: "revenue", ContactChannel: "#rev-data"}}},
	...
	actual := &DesiredState{
		Domains: []DomainResource{{Name: "revenue", Spec: DomainSpec{Description: "Revenue"}}},
		Teams:   []TeamResource{{Name: "analytics-engineering", Spec: TeamSpec{DomainRef: "revenue", ContactChannel: "#old-channel"}}},
		DataProducts: []DataProductResource{{
			Slug: "daily-orders",
			Spec: DataProductSpec{
				Name:              "Daily Orders",
				DomainRef:         "revenue",
				OwnerTeamRef:      "analytics-engineering",
				StewardPrincipal:  "bob",
				ContactChannel:    "#old-channel",
				Contract:          ProductContractSpec{DataGrain: "one row per customer"},
				SLO:               ProductSLOSpec{FreshnessSLO: "120m"},
				Outputs:           []string{"legacy_asset"},
	...
	var productUpdate *Action
	for i := range plan.Actions {
		if plan.Actions[i].ResourceKind == KindDataProduct {
			productUpdate = &plan.Actions[i]
			break
		}
	}
	require.NotNil(t, productUpdate)
	assert.Equal(t, OpUpdate, productUpdate.Operation)

	fields := map[string]bool{}
	for _, change := range productUpdate.Changes {
		fields[change.Field] = true
	}
	assert.True(t, fields["steward_principal"])
	assert.True(t, fields["contact_channel"])
	assert.True(t, fields["contract"])
	assert.True(t, fields["slo"])
	assert.True(t, fields["outputs"])
