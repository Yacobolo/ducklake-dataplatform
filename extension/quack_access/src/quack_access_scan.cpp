...
#include "quack_access_manifest.hpp"
...
unique_ptr<TableRef> QuackAccessScan::ReplacementScanFunction(
	...
	// Step 1: Check if a quack_access secret exists.
	// If not, this table isn't ours — return nullptr to let DuckDB handle it.
	auto secret = QuackAccessSecret::GetSecret(context);
	if (!secret) {
		return nullptr;
	}
	...
	if (!manifest) {
		// The API returned an error. Since the user has a quack_access secret,
		// they intend to use the platform — give a descriptive error.
		throw BinderException("quack_access: failed to resolve table '%s': %s",
	...
	from_ref->alias = "__quack_access_src";
		...
		for (auto &col : manifest->columns) {
			...
			auto expressions = Parser::ParseExpressionList(mask_sql);
				if (expressions.empty()) {
					throw BinderException("quack_access: failed to parse column mask for '%s': expression '%s'",
